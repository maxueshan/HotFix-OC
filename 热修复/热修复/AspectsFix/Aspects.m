...
static void aspect_prepareClassAndHookSelector(NSObject *self, SEL selector, NSError **error) {
    ...
    Class klass = aspect_hookClass(self, error);//替换 self 类的 forwardInvocation:的实现
    ...
    if (!aspect_isMsgForwardIMP(targetMethodIMP)) {
        ...
        SEL aliasSelector = aspect_aliasForSelector(selector); //添加新的方法,接收原方法实现,
        ...
        //_objc_msgForward 触发klassk类 selector方法的 消息转发机制, 去走forwardInvocation方法(已被重写)
        class_replaceMethod(klass, selector, aspect_getMsgForwardIMP(self, selector), typeEncoding);//触发消息转发(_objc_msgForward)
...
+ (instancetype)identifierWithSelector:(SEL)selector object:(id)object options:(AspectOptions)options block:(id)block error:(NSError **)error {
    NSCParameterAssert(block);
    NSCParameterAssert(selector);
    //block(需要执行的 js 实现 )->转换成 NSMethodSignature 方法签名
...
//执行block  (block已被转换成 methodSignature )// 外部的block中会调换用js新的实现
- (BOOL)invokeWithInfo:(id<AspectInfo>)info {
