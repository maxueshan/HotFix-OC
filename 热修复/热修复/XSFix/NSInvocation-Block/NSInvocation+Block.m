...
// https://www.jianshu.com/p/f5c92ff7bcd6
...
struct Block_literal_1 {
    ...
    int flags;
    int reserved;
    void (*invoke)(void *, ...); //函数指针,是block所执行代码的地址
    struct Block_descriptor_1 {
        unsigned long int reserved;     // NULL
        unsigned long int size;         // sizeof(struct Block_literal_1)
        // optional helper functions
         void (*copy_helper)(void *dst, void *src);     // IFF (1<<25)//是blcoK在拷贝时的回调，也就是说block在执行拷贝操作时我们可以拿到拷贝前和拷贝后的block。
         void (*dispose_helper)(void *src);             // IFF (1<<25)//是block在将被销毁时的回调。
        // required ABI.2010.3.16
         const char *signature;                         // IFF (1<<30)//是block的签名，这个签名类似OC方法的签名，它描述了block的返回值、参数信息。
        void* rest[1];
    } *descriptor;
    // imported variables
};
